"use strict";
/**
 * Пошаговое обучение на столе сборки — подсветка настоящих элементов
 * тулбара с карточкой рядом. Сделано по образцу «Поездок»/«Моих финансов»
 * (тот же assets/onboarding.js, та же разметка/CSS .onb-*), чтобы у
 * сервисов BurningHouse обучение выглядело и работало одинаково.
 *
 * Тур один, не два (в отличие от «Поездок» со списком+поездкой) — соло-стол
 * и стол в комнате используют один и тот же тулбар почти один в один,
 * разница только в паре виджетов (чат/присутствие — только в комнате).
 * shown() ниже сам молча пропускает шаг, если цели нет в разметке или она
 * скрыта — комнатные шаги (чат/присутствие) просто не покажутся на соло-
 * столе, отдельный список шагов ради этого заводить не нужно.
 *
 * Показывается один раз на устройство (localStorage, тот же ключ что и у
 * прогресса гостя — своей таблицы настроек тут тоже нет, заводить её ради
 * одного флага дороже, чем показать обучение второй раз на новом
 * устройстве). Повторно открывается кнопкой со знаком вопроса в шапке
 * стола (#tableHelpBtn, см. renderTable/renderRoomTable в app.js).
 *
...
/** Знак вопроса в шапке стола (#tableHelpBtn) — запускает тур заново. */
function openTour() {
  startTour("table");
}
