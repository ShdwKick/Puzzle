 ...
 * запрос против ~150 у текста. Зато titleFromImage не тянет за собой вообще
 * никакого текста (см. правку «По фото — только фото»): ни старого названия,
 * ни категории — смотрит только на саму фотографию. Поэтому и годится там,
 * где текста нет (импорт с Pexels без alt), и там, где он есть, но мешает:
 * у пазлов с болванкой вместо названия модель пересказывала болванку, а не
 * снимок.
 *
...
// Пул примеров-подписей (см. правку «Более оригинальные названия») — раньше
// все три промпта показывали модели ОДНИ И ТЕ ЖЕ два примера ("Рыжий кот на
// подоконнике"/"Горы на закате") при temperature:0 (см. ask ниже) —
// сочетание давало почти детерминированный результат, тянущий любую фотку
// кота/гор к этим же двум шаблонам. pickExamples берёт каждый раз случайную
// пару из более широкого набора — сама пара примеров перестаёт быть частью
// того, что модель копирует, а не иллюстрацией стиля.
// Примеры заодно ЗАДАЮТ планку живости (см. правку «Более живые названия»):
// модель копирует их стиль охотнее, чем следует описанию правил, поэтому
// список специально смешанный — есть и просто точные подписи, и с образом
// («Рыжий хозяин подоконника»). Раньше здесь стояли «Горы на закате» и
// «Ночные огни мегаполиса» — то самое, что следующая же строка правил
// запрещает как штамп: примеры спорили с инструкцией, и побеждали примеры.
const EXAMPLE_TITLES = [
  "Рыжий хозяин подоконника", "Тишина перед грозой", "Город просыпается",
  "Щенок в свежем снегу", "Маяк держит ночь", "Хруст осенней аллеи",
  "Стеклянные великаны", "Полевые ромашки", "Болид на прогретом асфальте",
  "Пустыня считает звёзды", "Туман застрял в соснах", "Львёнок учится рычать",
];
function pickExamples() {
  const pool = [...EXAMPLE_TITLES];
  const a = pool.splice(Math.floor(Math.random() * pool.length), 1)[0];
  const b = pool.splice(Math.floor(Math.random() * pool.length), 1)[0];
  return `"${a}", "${b}"`;
}
// Общий хвост правил — раньше был продублирован в каждом промпте почти
// дословно, только с разными примерами; вынесено, чтобы менять формулировку
// оригинальности в одном месте, а не в трёх синхронно.
const RULES_TAIL = () => `Верни СТРОГО JSON без пояснений, без markdown: {"ru":"","en":""}
Правила: 2-4 слова каждое, как подпись к картинке в каталоге (например ${pickExamples()}).
Название должно цеплять: приветствуются образ, деталь, лёгкая метафора или настроение сцены — но так, чтобы по нему было понятно, что на картинке. Лучше живая подпись, чем протокольное перечисление объектов.
Не описывай очевидное словами-затычками ("Красивый вид", "Природный пейзаж") и не лепи штампы по шаблону "X на закате"/"X крупным планом"/"Огни большого города". Цепляйся за то, что отличает ИМЕННО эту сцену: порода, цвет, свет, действие, место, время года, настроение.
Русское и английское — не подстрочник друг друга, а два самостоятельных названия об одном и том же кадре.
Английское — с большой буквы у каждого слова (Title Case). Без кавычек внутри строк, без точки в конце, без слов вроде "пазл"/"puzzle".`;
...
// Ни старого названия, ни категории — сознательно (см. правку «По фото —
// только фото»). Сначала убрали только название, и этого не хватило: при
// импорте с Pexels фото без alt получает название ИЗ ИМЕНИ КАТЕГОРИИ
// («Библиотека 1», см. Admin/assets/app.js), то есть категория — тот же
// самый старый текст, просто другим полем. Модель цеплялась уже за неё и
// выдавала «Космическая библиотека» независимо от того, что на снимке.
// Плюс подсказка категорией и сама по себе тянет к дежурному слову по теме,
// против которого написана вторая половина RULES_TAIL.
const VISION_PROMPT = () => `Придумай короткое название для пазла по этой фотографии — на русском и на английском.
Смотри ТОЛЬКО на саму фотографию: описывай то, что на ней действительно видно.
${RULES_TAIL()}`;
...
module.exports = function createGigaChat(options = {}) {
  ...
  async function ask(messages) {
    const res = await call(API_BASE + "/chat/completions", {
      ...
      // temperature:0 раньше — задача не факт-экстракшн, где нужна
      // повторяемость, а короткая творческая подпись; вместе с одними и
      // теми же примерами (см. EXAMPLE_TITLES выше) это и давало похожие
      // названия у разных фото. 0.9 — заметная вариативность, normalizeTitle
      // ниже всё равно отбраковывает пустой/слишком длинный результат.
  ...
  /** Название по САМОЙ фотографии: ни старого названия, ни категории в
   *  запрос не идёт (см. VISION_PROMPT выше — там же, почему категория тоже
   *  вредна). Дороже titleFromText на ~2000 токенов картинки, зато ничего не
   *  наследует от прежней болванки. */
