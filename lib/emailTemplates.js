...
/** roomless — фото загружали сразу на публикацию, без комнаты (см.
 *  server.js, POST /api/puzzles?publish=1): такого фото после отказа уже нет
 *  (модерация удаляет группу — лежать ему негде и попасть в него неоткуда),
 *  поэтому обещать «осталось в комнате, отправьте ещё раз» тут нельзя —
 *  вместо этого зовём загрузить заново на странице публикации. */
function publishRejected({ title, reason, link, roomless }) {
  const tail = roomless
    ? "Загрузить другое фото на публикацию можно на странице публикации."
  ...
  const html = shell({
    ...
    intro: `Ваше фото «${escapeHtml(title)}» не прошло модерацию для публикации в общую библиотеку.${reason ? ` Причина: ${escapeHtml(reason)}.` : ""} ${tail}`,
    buttonLabel: roomless ? "Опубликовать другое фото" : "Открыть комнату",
    link,
    footNote: "Это письмо отправлено, потому что вы отправляли это фото на публикацию в «Что собираем?».",
  });
