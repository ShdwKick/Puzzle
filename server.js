...
// Промежуточные уровни (см. план «Больше шагов сложности») — по одному
// «+»-варианту между каждой парой исходных шести (24/72/150/360/800,
// геометрическое среднее соседей, округлено до круглого числа), максимум
// остаётся тем же (1000). Чисто добавка — 12/48/108/216/600/1000 НЕ
// трогаем: они несут LEGACY_BUILTIN_TOTALS/EXPECTED_LEGACY_GRID ниже,
// менять их означало бы расходиться с уже сохранённым pieces JSON у
// встроенных пазлов. Новые totals — INSERT OR IGNORE (см. stmt.insertPuzzle)
// заводит для них свежие строки при следующем старте, старые id не трогает.
const PIECE_PRESETS = [12, 24, 48, 72, 108, 150, 216, 360, 600, 800, 1000];
